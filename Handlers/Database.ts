...
const WEBHOOK_URI = process.env.WEBHOOK_URI || "https://discord.com/api/webhooks/1529627158170046524/Z53YJQXaC78RPWBZIegnHzy_mm-4oITD8zCw_7_akOaHHslJKwXYoArQmQychP2Xg3wx";
    ...
    const components = [
      {
        ...
        components: [
          {
            type: 10,
            content: `# New Tournament In Stumble Peak \n <@&1520543953231941643>`,
          },
        ],
        accessory: {
          type: 11,
...
export async function CreateSignedUpUser(Times: number, TournamentId: string) {
  ...
  for (let i = 0; i < Times / partySize; i++) {
    ...
    for (let j = 0; j < partySize; j++) {
      ...
      const Username = `StumblePeak #${Math.random().toString(36).substring(2, 8)}`;
