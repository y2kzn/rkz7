...
UserCollection.index({ AccessToken: 1 }, { unique: true });
UserCollection.index({ UserId: 1 }, { unique: true });
UserCollection.index({ DeviceIdentifier: 1 }, { unique: true });
UserCollection.index({ ClientToken: 1 });
UserCollection.index({ RefreshToken: 1 });
UserCollection.index({ ExpireAt: 1 });
UserCollection.index({ Nickname: 1 }, { unique: true });
